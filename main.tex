...
summary of LSST, please see the LSST Overview paper\footnote{arXiv:0805.2366, http://ls.st/2m9}. 
...
\B{The main LSST data products are illustrated in Figure~\ref{fig:Detail0} (see Appendix for 
...
standards\footnote{For example, VOEvent, see http://ls.st/4tt} within 60 seconds of observation. Level 1 data products are discussed in \S~\ref{sec:level1}.
...
\B{For completeness, $\chi^2$ is defined as:
%
\begin{equation}
...
\B{Some \DIASources will be caused by background fluctuations; with $transSNR = \transSNR$, 
...
\B{collected during previous 12 months. Hence, the computed parameters for \DIAObjects have a ``memory'' of past data that does not extend beyond this cutoff\footnote{This restriction is removed when Level 1 processing is
...
\item For all \DIAObjects overlapping the field of view, \R{which do not} \B{including those that} have an associated 
...
\B{A gravitational keyhole is a region of space where Earth's gravity would modify the orbit of a passing asteroid 
...
\B{in general, a time-domain object is not necessarily the same astrophysical object as a static-sky object, even if the two are 
...
\B{apFlux} & float & nmgy & Calibrated aperture flux. Note that this actually measures 
the flux {\em difference} between the template and the visit image. \\ 

\B{apFluxErr} & float & nmgy &  Estimated uncertainty of \texttt{apFlux}. \\  
...
\B{psRadec} & double[2] & degrees & Centroid for point source model. \\ 
...
\B{trailRadec} & double[2] & degrees & Centroid for trailed source model. \\ 
...
\B{dipMeanFlux} & float & nmgy & Maximum likelihood value for the mean absolute flux of the two lobes for a dipole model\footnote{A {\em Dipole Model} attempts to fit a (PSF-convolved) model of two point sources, with fluxes
...
\B{dipFluxDiff} & float & nmgy & Maximum likelihood value for the difference of absolute fluxes of the two lobes for a dipole model. 
\\ 

\B{dipRadec} & double[2] & degrees & Centroid for dipole model. \\ 

\B{dipLength} & float & arcsec & Maximum likelihood value for the lobe separation in dipole model. \\ 

\B{dipAngle} & float & degrees & Maximum likelihood fit of the angle between the meridian through the centroid and the dipole direction (bearing, from negative to positive lobe). \\ 

\B{dipCov} & float[15] & various & Covariance matrix of dipole model parameters. \\ 

\B{dipLnL} & float & ~ & Natural $log$ likelihood of the observed data given the dipole source model. \\ 

\B{dipChi2} & float & ~ & $\chi^2$ statistic of the model fit. \\

\B{dipNdata} & int & ~ & The number of data points (pixels) used to fit the model. \\
...
\B{IxxPSF} & float & nmgy asec$^{2}$ & Adaptive second moment for the PSF. \\

\B{IyyPSF} & float & nmgy asec$^{2}$ & Adaptive second moment for the PSF. \\

\B{IxyPSF} & float & nmgy asec$^{2}$ & Adaptive second moment for the PSF. \\
...
\B{spuriousness} & float & ~ & A measure of spuriousness, computed using information\footnote{\B{The computation
...
Some fast-moving, trailed, sources may be due to passages of nearby asteroids. Their trails may exhibit significant curvature. \R{For trails longer than a certain treshold (to be determined by start of Operations), we will fit a curved trail model which will be included with the alert (but likely stored in a separate table).}
\B{While we do not measure the curvature directly, it can be inferred by examining the length of the trail, the trailed model covariance matrices, and the adaptive shape measures. Once curvature is suspected, the users may fit curved trail models to the cutout provided with the alert.}
...
pmParallaxLnL & float & ~ & Natural log of the likelihood of the linear proper motion-parallax fit\footnote{{\tt radec}, {\tt pm}, and {\tt parallax} will all be simultaneously fitted for. \R{{\tt positionMotionLnL} is the logarithm of the likelihod of their maximum-likelihood estimates.}}. \\
...
\B{psFluxChi2} & float[ugrizy] & ~ & $\chi^2$ statistic for the scatter of {\tt psFlux} around {\tt psFluxMean}. \\

\B{psFluxNdata} & int[ugrizy] & ~ & The number of data points used to compute {\tt psFluxChi2}. \\
...
\B{There will be three main advantages of \DB produced during Data Release processing, compared to 
``living'' \DB: 
...
Older versions of the \DB\ \B{produced during Data Release processing} will be archived following the same rules as for the \DR{}s. The most recent DR, and the penultimate data release will be kept on disk and loaded into the database. Others will be archived to tape and made available as bulk downloads. See \S~\ref{sec:retention} for more detail.
...
\item Science Data:
    ...
    \item \B{A matching \DIAObject from the latest Data Release, if it exists, and its \DIASource records}
    \end{itemize}
...
The master list of \Objects in Level 2 will be generated by associating and deblending the list of single-epoch \DIASource detections and the lists of sources detected on coadds. We plan to build coadds designed to maximize depth ({\em ``deep coadds"}) and coadds designed to achieve a good combination of depth and seeing ({\em ``best seeing coadds"}), \B{unless
...
\begin{enumerate}
    \item {\em Single \R{Frame} \B{Visit} Processing}: Raw exposures are reduced to {\em calibrated visit exposures}, and \Sources are independently detected, deblended, and measured on all visits. Their measurements (instrumental fluxes and shapes) are stored in the \Source table.
...
catalogs\footnote{\B{Note that \Sources are not considered when generating the \Object list (given the large
...
To enable easy identification of such \Sources, the nearest \Object associated with each \Source, if any, will be recorded.}}.
    ...
    \item \B{{\em Coadd object characterization}.  Object properties such as adaptive moments and aperture fluxes will be measured on the coadds.  These will be stored in additional columns in the \Object table.  Models fit in multi-epoch object characterization will also be fit first to the coadds and stored.} 
...
\B{Measurements will be performed on both direct images and difference images.}
...
%\B{including the so-called star-galaxy separation (estimation of the probability that a source is resolved, given the PSF).} 
\B{For example, although a simple extendedness characterization is present in the Source table, star-galaxy separation (an estimate of the probability that a source is resolved, given the PSF) will be better characterized by MultiFit.}
...
Forced photometry will be performed on all visits, for all \Objects, \B{using both direct images and difference images}. 
...
\B{Ixx} & float & nmgy asec$^{2}$ & Adaptive second moment of the source intensity. See Bernstein \& Jarvis (2002) for detailed discussion of all adaptive-moment related quantities\footnote{Or \url{http://ls.st/5f4} for a brief summary.}. \\

\B{Iyy} & float & nmgy asec$^{2}$ & Adaptive second moment of the source intensity. \\

\B{Ixy} & float & nmgy asec$^{2}$ & Adaptive second moment of the source intensity. \\

\B{Icov} & float[6] & nmgy$^{2}$ asec$^{4}$ & {\tt Ixx}, {\tt Iyy}, {\tt Ixy} covariance matrix. \\
...
\R{E1} & float[ugrizy] & ~ & Adaptive $e_1$ shape measure. See Bernstein \& Jarvis (2002) for detailed discussion of all adaptive-moment related quantities\footnote{Or \url{http://ls.st/5f4} for a brief summary.}. \\

\R{E2} & float[ugrizy] & ~ & Adaptive $e_2$ shape measure. \\

\R{E1E2cov} & float[ugrizy][3] & ~ & {\tt E1}, {\tt E2} covariance matrix. \\

\R{mSum} & float[ugrizy] & ~ & Sum of second adaptive moments. \\

\R{mSumSigma} & float[ugrizy] & ~ & Uncertainty in {\tt mSum} \\
...
\B{apFlux} & float & nmgy & Calibrated aperture flux. \\ 
...
\R{psXY} & float[2] & pixels & Point source model: $(column, row)$ position of the object on the CCD. \\

\R{psCov2} & float[6] & various & Point-source model covariance matrix for {\tt psRadec} and {\tt psFlux}.\\ 
...
\R{E1} & float & ~ & Adaptive $e_1$ shape measure. \\

\R{E2} & float & ~ & Adaptive $e_2$ shape measure. \\

\R{E1E2cov} & float[3] & ~ & {\tt E1}, {\tt E2} covariance matrix. \\

\R{mSum} & float & ~ & Sum of second adaptive moments. \\

\R{mSumSigma} & float & ~ & Uncertainty in {\tt mSum} \\

\R{m4} & float & ~ & Fourth order adaptive moment. \\
...
\B{extendedness} & float & ~ & A measure of extendedness, computed using a combination of available moments
...
\B{psFlux} & float & nmgy & Point source model flux on direct image, if performed.\\ 

\B{psFluxErr} & float & nmgy & Point source model flux error,  stored to 1\% precision.\\
...
There could also be L3 image data products; for example, user-generated coadds with special selection criteria or stacking algorithms \B{(eg. the so-called shift \& stack algorithm for detecting moving objects)}.
...
\item If the original code was written in another language or another data processing framework, the project 
\R{will have the resources to rewrite} \B{may consider rewriting} it to required LSST standards.
...
document\footnote{See http://ls.st/LDM-151} LDM-151. 
}
...
\begin{figure}[!t]
    ...
    \vskip -0.1in 
    \caption{Illustration of the conceptual design of LSST science pipelines for image processing.\label{fig:Detail1}}    
\end{figure}
